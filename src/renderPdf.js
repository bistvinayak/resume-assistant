...
async function measureResumePdf(resume) {
  return new Promise((resolve, reject) => {
    ...
    if (Array.isArray(c.links) && c.links.length) {
      doc.fontSize(9).font('Helvetica').fillColor('#1a6ed8').text(c.links.join('  |  '), { align: 'center' });
    }
    doc.fillColor('#000000');
    ...
    if (Array.isArray(resume.experience) && resume.experience.length) {
      ...
      for (const job of resume.experience) {
        ...
        doc.fontSize(11).font('Helvetica-Bold').text(titleLine, { continued: !!job.dates });
        if (job.dates) {
          doc.font('Helvetica-Oblique').fontSize(9).fillColor('#666666').text(`    ${job.dates}`, { align: 'right' });
          doc.fillColor('#000000');
        }
        if (job.tagline) {
          doc.fontSize(9).font('Helvetica-Oblique').fillColor('#666666').text(job.tagline);
          doc.fillColor('#000000');
        }
        for (const b of job.bullets || []) {
          const text = typeof b === 'string' ? b : (b.text || '');
          doc.fontSize(10).font('Helvetica').text(`•  ${text}`, { indent: 12, lineGap: 1.5 });
        }
      }
    }
    ...
    if (Array.isArray(resume.projects) && resume.projects.length) {
      ...
      for (const p of resume.projects) {
        doc.moveDown(0.2);
        doc.fontSize(10.5).font('Helvetica-Bold').text(p.name || '');
        if (p.description) doc.fontSize(10).font('Helvetica').text(p.description, { lineGap: 1.5 });
      }
    }
    ...
    if (Array.isArray(resume.education) && resume.education.length) {
      ...
      for (const e of resume.education) {
        ...
        doc.fontSize(10.5).font('Helvetica-Bold').text(e.degree || '', { continued: !!e.dates });
        if (e.dates) {
          doc.font('Helvetica').fontSize(9).fillColor('#666666').text(`    ${e.dates}`, { align: 'right' });
          doc.fillColor('#000000');
        }
        if (e.school) doc.fontSize(10).font('Helvetica').text(e.school);
      }
    }
